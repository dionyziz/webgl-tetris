...
//Attributes coming from buffers
...
void main() {
    //Calculate the new position and the new normal of the vertex. No projection yet
    pPosition = pmatrix * vec4( vPosition, c_one );
    pNormal = mat3( pmatrix[ 0 ].xyz, pmatrix[ 1 ].xyz, pmatrix[ 2 ].xyz ) * vNormal;
    //Use them to caclulate the color at the vertex
    
    //TODO use the proper matrix for calculating normals instead of normalizing
    vColor = spotlight( normalize( pNormal ), pPosition );
    //Do the camera transformation and perspective projection
